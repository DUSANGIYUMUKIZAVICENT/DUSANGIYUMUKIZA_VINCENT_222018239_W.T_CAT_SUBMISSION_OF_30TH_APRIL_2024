...
error_reporting(0);
include 'connect.php';
$sel=$con->query("SELECT DISTINCT matches.*,referees.*,team.* FROM  matches JOIN referees ON(matches.ref_id=referees.ref_id) JOIN team ON(matches.t1_id=team.t_id) WHERE matches.action='ON'");
 ?>
<!DOCTYPE html>
<html>
<head>
...
<body>
	...
	<center><br>
		...
		<tr><th colspan="5">MACH REPORT</th><th colspan="2">Action</th></tr><tr>
		...
		while ($data=$sel->fetch_assoc()) 
		{
			...
			<td><a href='edit.php?mt=$mc'> Edit</a></td>
			<td><a href='delete.php?mt=$mc'>Delete</a></td>
			</tr>";
			$a++;
		}
		 ?>
	</table></form><button onclick="history.back()">Back</button><button onclick="window.location='deleted.php'">View Deleted match</button><br>
	<br><br>
